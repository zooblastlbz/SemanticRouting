...
class AdaFuseDiTPipeline(DiffusionPipeline, FromSingleFileMixin):
    ...
    def __call__(
        ...
        # Match training precision for weights
        ...
        # Use the same precision as training: weight_dtype.
        # During training the LLM self-attention mask is all ones (no PAD masking), so build the same mask here for consistency.
        ...
        # === AdaFuseDiT core: extract multi-layer text features ===
        # LLM inference mirrors training, using weight_dtype
        with torch.no_grad():
            llm_output = self.llm(
                input_ids.to(self.device),
                llm_attention_mask,
        ...
        # 6. Denoising loop - match training without extra autocast
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                ...
                # === AdaFuseDiT forward: pass in multi-layer text features ===
                # Match training by converting latent to weight_dtype
                noise_pred = self.transformer(
